...
def load_math_results_for_comparison(math_paths):
    ...
    - {"mid_res": Path(".../ostios_test_summary.csv")}
    - {"mid_res": {"train": Path(...), "val": Path(...), "test": Path(...)}}
    """
    math_frames = []
    missing_math_files = []
...
def build_comparison_agg_df(comparison_raw):
    ...
    For fair IA-vs-math comparison, math rows are restricted to IMG IDs that
    appear in IA rows for the same target resolution.
    """
