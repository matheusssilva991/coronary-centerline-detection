...
RUN_NAME="${RUN_NAME:-artery_segmentation_optimization_${SPLIT}${SAMPLE_SIZE}_$(date +%Y-%m-%d_%H-%M-%S)}"
USE_GPU="${USE_GPU:-1}"

EXTRA_ARGS=()
if [[ -n "${OPTIMIZATION_VARIANTS:-}" ]]; then
  EXTRA_ARGS+=(--optimization-variants "$OPTIMIZATION_VARIANTS")
fi
